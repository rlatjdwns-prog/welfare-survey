...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      try{ const cfg=await apiGet("config"); if(cfg.questions)setQuestions(cfg.questions); if(cfg.company)setCompany(cfg.company); if(cfg.intro)setIntro(cfg.intro); if(cfg.lottery)setLottery(cfg.lottery); }catch{}
      setPhase("intro");
    })();
  },[]);
  ...
  const next=async()=>{
    ...
    const alreadySubmitted=localStorage.getItem("survey_submitted")==="true";
    const won=lottery.enabled&&!alreadySubmitted&&Math.random()*100<lottery.probability;
  ...
  const saveWinner=async()=>{
    ...
    try{await apiPost("saveWinner",{name:winName,contact:winContact,prize:lottery.prize,date:new Date().toLocaleString("ko-KR")});}catch{}
    setWinSaved(true);
  };
        ...
        <div className="card" ref={cardRef}>
          ...
          {phase==="intro" && (
            <>
              ...
              {lottery.enabled && (
                <div style={{background:"var(--gold-bg)",border:"1px solid var(--gold)",borderRadius:"12px",padding:"12px 16px",marginBottom:"24px",fontSize:"13px",color:"var(--gold)",fontWeight:500}}>
                  🎁 설문 완료 후 즉석 추첨 — {lottery.prize}
                </div>
              )}
            ...
            <>
              ...
              {lottery.enabled && (
                <div className="lottery-banner" onClick={()=>go("lottery")}>
                  ...
                  <div className="lottery-banner-text">
                    <div className="lottery-banner-title">즉석 추첨 참여하기 →</div>
                    <div className="lottery-banner-desc">{lottery.description}</div>
                  </div>
                </div>
              )}
